...
void AppWindow::render()
{
	...
	GraphicsEngine::get()->getRenderSystem()->getImmediateDeviceContext()->clearRenderTargetColor(this->m_swap_chain, 0.75f, 0, 1, 1);
...
void AppWindow::updateCamera()
{
	...
	Vector3D new_pos = m_world_cam.getTranslation() + world_cam.getZDirection() * (m_forward*0.10f);
	new_pos = new_pos + world_cam.getXDirection() * (m_rightward*0.10f);
	...
	int heigth = (this->getClientWindowRect().bottom - this->getClientWindowRect().top);

	m_proj_cam.setPerspectiveFovLH(1.57, ((float)width / (float)heigth), 0.1f, 100.0f);
}
...
void AppWindow::onMouseMove(const Point & mouse_pos)
{
	...
	m_rot_x += (mouse_pos.m_y - (heigth / 2.0f)) * m_delta_time*0.1f;
	m_rot_y += (mouse_pos.m_x - (width / 2.0f)) * m_delta_time*0.1f;

	InputSystem::get()->setCursorPosition(Point(width / 2.0f, heigth / 2.0f));
